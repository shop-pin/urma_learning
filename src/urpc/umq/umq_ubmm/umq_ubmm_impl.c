...
uint64_t umq_ubmm_create_impl(uint64_t umqh, uint8_t *ubmm_ctx, umq_create_option_t *option)
{
    ...
    *(uint32_t *)export_param.deid = dev_ctx->ubmm_eid;
    ...
    UMQ_VLOG_INFO("ipc create umq_id: %u\n", tp->umq_id);
    ...
    tp->ubmm_ctx = dev_ctx;
    tp->ref_cnt = 1;
    UMQ_VLOG_INFO("create ubmm tp succeed\n");
...
int32_t umq_ubmm_destroy_impl(uint64_t umqh_tp)
{
    ...
    umq_dec_ref(tp->ubmm_ctx->io_lock_free, &tp->ubmm_ctx->ref_cnt);
    free(tp);
    return UMQ_SUCCESS;
}
...
static ALWAYS_INLINE umq_buf_t *umq_prepare_rendezvous_data(umq_ubmm_info_t *tp, umq_buf_t *qbuf, uint16_t *msg_id)
{
    ...
    if (send_buf == NULL) {
        UMQ_VLOG_ERR("alloc rendezvoud buf failed\n");
        return NULL;
    }
...
umq_buf_t *umq_ubmm_plus_dequeue_impl(uint64_t umqh_tp)
{
    ...
    if (buf == NULL) {
        UMQ_VLOG_DEBUG("ub dequeue return nothing\n");
    }
    ...
    if (polled_buf == NULL) {
        UMQ_VLOG_DEBUG("umq_shm_qbuf_dequeue return nothing\n");
    } else if (rendezvous) {
        ...
        if (umq_ub_read(tp->ub_handle, polled_buf, imm_data) != UMQ_SUCCESS) {
            UMQ_VLOG_DEBUG("send read failed\n");
