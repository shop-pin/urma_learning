...
3、host1和host2各自get_lock一把读写锁，调用get_lock接口
4、host1和host2循环执行加锁(LOCK_EXCLUSIVE和LOCK_SHARED间隔执行)解锁，调用trylock，unlock接口
...
class Test(UBUSFeature):
    ...
    def test_dlock_rw_trylockunlock_normal(self):
