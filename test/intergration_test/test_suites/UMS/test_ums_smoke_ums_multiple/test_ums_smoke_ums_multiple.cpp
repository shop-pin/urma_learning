...
static int run_test()
{
    ...
    if (ctx->app_id == PROC_2) {
        ...
        exec_cmd(serv_cmd, MAX_EXEC_CMD_RET_LEN, "for i in $(seq 5585 5594), do nohup ums_run qperf -lp ${i} & done");
    }
    sync_time("----------------------------1");
    if (ctx->app_id == PROC_1) {
        char clnt_cmd[MAX_EXEC_CMD_RET_LEN];
        exec_cmd(clnt_cmd, MAX_EXEC_CMD_RET_LEN, "for i in $(seq 5585 5594), do nohup ums_run qperf %s -lp ${i} -m 8192 -t 0 tcp_bw 2>&1 & done", serv_ip);
    }
    sync_time("----------------------------2");
